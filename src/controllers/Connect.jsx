...
function Connect({sendProvider, sendWallet}) {
    ...
    async function connect() {
        ...
        if (!window.ethereum) {
            window.alert("Install a QUAI Wallet")
            if(confirm("Download Pelagus")) document.location = 'https://pelaguswallet.io/';
        }
                    ...
                    <div className="container flex justify-between space-x-4 lg:space-x-16 items-start">
                        ...
                        <nav className="hidden md:flex lg:flex justify-between flex-1 pl-10">
                            <div className="flex items-center lg:text-md space-x-2 lg:space-x-8 drop-shadow-xl text-white">
                                <a href="https://qu.ai/" target="_blank" className="hover:text-gega-sky transition duration-500 tracking-widest"><i className="fa-brands fa-hive"></i> QUAI</a>
                                <a href="https://faucet.quai.network/" target="_blank" className="hover:text-gega-sky transition duration-500 tracking-widest text-blue-500"><i className="fa-solid fa-faucet"></i> Faucet</a>
